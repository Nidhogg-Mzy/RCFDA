...
module_directory = os.path.abspath('/data/clhuang/5212/exams-qa/')
sys.path.append(module_directory)
# print(sys.path)

from src.examsqa.models_multiple_choice import (
...
def set_row_and_column_random_mask(model):
        ...
        for name, param in model.named_parameters():
            ...
            if np.random.randint(0, 2) or param.shape[0] < param.shape[1]:  # we randomly choose a column
                ...
                masks[name][row_indices] = True

        # LOGGER.info(f'Non-Masked params amount: {int(np.sum([np.sum(mask.numpy()) for mask in self.masks.values()]))}. '
        #             f'Total params: {total_params}')
                
        print(f'Non-Masked params amount: {int(np.sum([np.sum(mask.numpy()) for mask in masks.values()]))}. '
                    f'Total params: {total_params}')
        return masks
...
def train(args, train_dataset, model, tokenizer, masks, max_grad_norm=1.0):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        print("args.tb_log_dir: " + str(args.tb_log_dir))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    # print('=========================  args.task_name is: ' + str(args.task_name) + '  =========================\n\n\n\n\n')
    processor = (
    ...
    # print('=========================  processor is: ' + str(processor) + '  =========================\n\n\n\n\n')
    if evaluate:
        cached_mode = "dev"
    elif test:
        cached_mode = "test"
    ...
    # later can access the 1st example by  `input_ids, input_mask, segment_ids, label_ids, answers_mask = dataset[0]`
...
def main():
    ...
    if args.do_train:
        ...
        masks = set_row_and_column_random_mask(model)
        global_step, tr_loss, best_steps = train(args, train_dataset, model, tokenizer, masks)
