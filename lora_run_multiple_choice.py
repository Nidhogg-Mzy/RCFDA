...
ALL_MODELS = sum(
    (
        # tuple(conf.pretrained_config_archive_map.keys())
        # for conf in (BertConfig, XLNetConfig, RobertaConfig)
        tuple(BERT_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()), # for BertConfig
        tuple(ROBERTA_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()), # for RobertaConfig
        tuple(XLNET_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()), # for XLNetConfig
    ),
    (),
)
...
def train_with_adapter(args, train_dataset, tokenizer):
    model = transformers.BertForMultipleChoice.from_pretrained(args.ckpt)
    adapters.init(model)
    config = SeqBnInvConfig()
    model.add_adapter("lang_adapter", config=config)

def train_with_lora(args, train_dataset, tokenizer):
    model = transformers.BertForMultipleChoice.from_pretrained(args.ckpt)
...
def main():
    ...
    parser.add_argument(
        "--train_with_adapter",
        action="store_true",
        help="Whether to train with adapter",
    )
    parser.add_argument(
        "--train_with_lora",
        action="store_true",
    ...
    parser.add_argument(
        "--ckpt",
        default="/data/zmengaf/exams-qa/train_lr_3e-7/checkpoint-56500",
        type=str,
        required=False,
        help="Path to pre-trained model or shortcut name selected in the list: "
        + ", ".join(ALL_MODELS),
    ...
    if args.train_with_adapter:
        model = transformers.BertForMultipleChoice.from_pretrained(args.ckpt)
        tokenizer = transformers.BertTokenizer.from_pretrained(args.ckpt)
        adapters.init(model)
        config = SeqBnInvConfig()
        model.add_adapter("lang_adapter", config=config)
